...
import sys
from touch_capture import BAUD_RATE
...
def main(port):
    ...
    while rounds > 0:
        ...
        cv2.putText(frame, "SPACE: play  |  Q: quit", (10, frame.shape[0] - 20),
            ...
            with serial.Serial(port, 115200, timeout=1.0) as ser:
                if ser.in_waiting > 0:
                    ...
                    if data == "START":
                        ...
                        # simple file logging
                        with open("rps_results.csv", "a") as f:
                            f.write(f"{player_move},{computer_move},{winner}\n")
                    else:
...
def playerSelfReport(result, port):
    reported = False
    try:
        with serial.Serial(port, 115200, timeout=1.0) as ser:
